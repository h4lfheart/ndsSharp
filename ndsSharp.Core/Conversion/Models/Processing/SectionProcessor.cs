...
public class SectionProcessor(MDLModel Model)
{
    ...
    public void ProcessCommand(MDLRenderCommand command)
    {
         switch (command.OpCode)
        {
            ...
            case RenderCommandOpCode.SKINNNG_EQUATION:
            {
                /*var storeIndex = command.Parameters[0];
                var numTerms = command.Parameters[1];

                var finalMatrix = new Matrix4x4();
                var termParameterData = command.Parameters[2..].Chunk(3);
                foreach (var termParameters in termParameterData)
                {
                    var stackIndex = termParameters[0];
                    var invBindIndex = termParameters[1];
                    var weight = termParameters[2] / 256f;
                    
                    var matrix = ProcessingUnit.MatrixStack[stackIndex];
                    matrix *= Model.InvBindMatrices[invBindIndex].To4x4();
                    matrix *= weight;

                    finalMatrix += matrix;
                }

                ProcessingUnit.MatrixStack[storeIndex] = finalMatrix;*/
